...
\addto\captionsenglish{%
   \renewcommand\listfigurename{aa}}
%\renewcommand*\listfigurename{aa}
...
%				}}
%				{\Large\bf \textbf {ĐỒ ÁN TỐT NGHIỆP}}
...
%				}}
				...
				\text{thang.ld163830@sis.hust.edu.vn}
%				\vskip 0.5cm
%				{\bf \textbf{LAI ĐỨC THẮNG}} \\
%				thang.ld163830@sis.hust.edu.vn
...
Chúng ta sẽ tiến hành huấn luyện mô hình dựa trên bộ dữ liệu được tạo ra ở trên.  Mô hình mà em sử dụng trong bài toán này là RetinaNet; trong đó backbone được sử dụng trong mạnh kim tự tháp đặc trưng FPN là ResNet50 được pretrained trên bộ dữ liệu nổi tiếng ImageNet phục vụ cho việc trích xuất các đặc trưng trong ảnh. Tổng số ảnh được đưa vào mô hình là 4505 ảnh,  trong đó lấy ra 20\% số ảnh được đưa vào tập test để đánh giá mô hình sau mỗi vòng lặp. Tổng số vòng luyện mạng là 50 vòng, thời gian luyện xấp xỉ 8 giờ (chạy trên CPU 4 cores,  GPU  Tesla V100).  Kết quả việc đánh giá mô hình sao mỗi vòng lặp được thể hiện thông qua giá trị \textit{Average precison} (hình \ref{fig:mAPplot}) %Add ảnh vào
\begin{figure}
\centering
...
\begin{figure}[!h]
    ...
    \includegraphics[width=\linewidth]{Images/detect_fail/Img_RSKA003603_0_r2304_c3584}
        \subcaption[short for lof]{Vùng đất gây hiểu nhầm}
%        \rule{\linewidth}{3cm}
        \label{fig:subfig1}
    \end{subfigure}%
    \quad
    ...
    \includegraphics[width=\linewidth]{Images/detect_fail/Img_RSKA003603_0_r2176_c1024}
        \subcaption[short for lof]{mô hình nhận dạng nhầm bụi cỏ}
%        \rule{\linewidth}{3cm}
        \label{fig:subfig2}
    \end{subfigure}
